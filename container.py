...
class PriorityQueue(Container):
    ...
    def add(self, item: Any) -> None:
        ...
        q = self._queue
        len_ = len(q)
        if len_ == 0:  # just simply append
            q.append(item)
            return None

        # use inequalities to represent priority
        # if len_ >= 1, and item <= first item, add to front
        if not self._higher_priority(item, q[0]):
            q.insert(0, item)
            return None

        i = 0
        while i < len_ - 1:  # loop run iff len_ > 1
            # q[i] < item <= q[i+1]
            if self._higher_priority(item, q[i]) and \
                    not self._higher_priority(item, q[i + 1]):
                q.insert(i + 1, item)
                return None
            i += 1

        # we now know that for len_ == 1, item must > first item, then append;
        # also include case where not (q[-2] < item <= q[-1]).
        # i.e., item > q[-1], then append.
        if i == len_ - 1:
            q.append(item)
